        ...
        const renderItem = ({ item,index }) => {
            return (
                <View style={{ padding: 10, alignItems: 'center', }}>
                    <View style={{ position: "absolute", top: 17, left: 10, height: 30, width: 30, borderRadius: 100, backgroundColor: index % 2 ?"pink":"gold"}}/>
                    <Feather name={item.icon} color="#222" size={25} />
                    <Text style={{ fontSize: 13, fontWeight: 'bold', color: "#333" }}>{item.label}</Text>
                </View>
            )
        }
        return (
